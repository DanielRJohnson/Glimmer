...
func TestTypeofErrors(t *testing.T) {
	tests := []struct {
		...
		{"x", "[1,2]: identifier not found: x"},
		{"[1, 2, 3, 4.2]", "[1,1]: array must have matching types"},
		{`{"a": 1, "b": fn(x: int) -> int { x }}`, "[1,1]: dict must have matching value types"},
		{"fn(x: none, y: int) -> none { }", "[1,3]: param can not be none type"},
		{"if true { 1 } else { 2.2 }", "[1,3]: if branches must match types"},
		{"arr = [1,2,3,4]; arr[3.2];", "[1,21]: index of array must be int"},
		{`dic = {"a": 1}; dic[3.2];`, "[1,20]: index of dict must be string"},
		{"fn(a: int, b: int) -> int { if true { false } else { false } }", "[1,3]: function body type does not match return type"},
		{"fn() -> int { 1 }(true)", "[1,18]: invalid number of arguments in call"},
		{"fn(x: int) -> int { x } (false)", "[1,25]: param type mismatch for param 1 in call"},
		{"-[1,2,3,4]", "[1,1]: input to prefix op '-' must be numeric"},
		{"![1,2,3,4]", "[1,1]: input to prefix op '!' must be numeric"},
		{"[] + []", "[1,4]: infix operator for 'array[none] + array[none]' not found"},
		{"len(1, 2)", "[1,4]: Incorrect num of arguments to len, got=2"},
		{"len(1)", "[1,4]: Argument to len must be array or string, got=int"},
		{"head(1, 2)", "[1,5]: Incorrect num of arguments to head, got=2"},
		{"head(1)", "[1,5]: Argument to head must be array, got=int"},
		{"tail(1, 2)", "[1,5]: Incorrect num of arguments to tail, got=2"},
		{"tail(1)", "[1,5]: Argument to tail must be array, got=int"},
		{"x = [1,2,3,4,5]; slice(x)", "[1,23]: Incorrect num of arguments to slice, got=1"},
		{"x = [1,2,3,4,5]; slice(1, 2, 3)", "[1,23]: Argument 1 to slice must be array, got=int"},
		{"x = [1,2,3,4,5]; slice(x, true, 3)", "[1,23]: Argument 2 to slice must be int, got=bool"},
		{"x = [1,2,3,4,5]; slice(x, 2, true)", "[1,23]: Argument 3 to slice must be int, got=bool"},
		{"push()", "[1,5]: Incorrect num of arguments to push, got=0"},
		{"push(1, 2)", "[1,5]: Argument 1 to push must be array, got=int"},
		{"push([1,2,3], true)", "[1,5]: Argument 2 to push must be match Argument 1's held type: int, got=bool"},
		{"pop(1, 2)", "[1,4]: Incorrect num of arguments to pop, got=2"},
		{"pop(1)", "[1,4]: Argument 1 to pop must be array, got=int"},
	}
